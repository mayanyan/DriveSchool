 ...
 *  @param action 回调方法
 */
- (void)addHeaderWithTarget:(id)target action:(SEL)action color:(UIColor *)color
{
    // 1.创建新的header
    if (!self.header) {
        MJRefreshHeaderView *header = [[MJRefreshHeaderView alloc]initWithFrame:CGRectMake(110, 0, CURRENT_WIDTH, 110)];
